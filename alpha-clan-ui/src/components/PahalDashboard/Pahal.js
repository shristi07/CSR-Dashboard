...
const Pahal = ({ crumbs }) => {
  const { donationData, volunteerData, fundraiserData } = useSelector(
    (state) => ({
      ...state.pahalReducer,
    })
  );
              ...
              {volunteerData.map((event) => (
                <div className="card-widget-basic event">
                  ...
                  <div>
                    <a
                      className="mailto"
                      href={`mailto:${event?.contact?.mail}`}
                    >
              ...
              {donationData.map((drive) => (
                <div className="card-widget-basic drive">
                  ...
                  <div className="date">
                    {!drive?.ends
                      ? `Starts ${drive?.starts}`
                      : `${drive?.starts}  -  ${drive?.ends}`}
                  </div>
                  <div>
                    <a
                      className="mailto"
                      href={`mailto:${drive?.contact?.mail}`}
                    >
              ...
              {fundraiserData.map((fundraiser) => (
                <div className="card-widget-basic fundraiser">
                  ...
                  <div className="details card-align">
                    <div className="goal">{fundraiser.fund_aim}</div>
                    <div className="raised">{fundraiser.collected}</div>
                  </div>

                  <div>{fundraiser?.comment}</div>
                  <div className="supporters">
                    {fundraiser?.total_supporters}
                  </div>
                  <div className="ends">{fundraiser?.ends}</div>
                  <div>
                    <a
                      className="mailto"
                      href={`mailto:${fundraiser?.contact?.mail}`}
                    >
