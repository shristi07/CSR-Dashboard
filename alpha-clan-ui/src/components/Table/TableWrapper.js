...
const TableWrapper = ({
  ...
  useEffect(() => {
    if (enableCsvExport) {
      const myColumns = columns.filter(({accessor}) => !filterColumnsFromCsv.includes(accessor))
  ...
  }, [rows, columns, enableCsvExport, setCsvData, getCSVData, filterColumnsFromCsv, overrideCSVData]);
  useEffect(() => {
    ...
    (rows) => rows.map((row, index) => {
      ...
      return (
        ...
        >
          {overrideCellsData
            ? row.cells.map((cell) => (
              <td className={`${cell.column.id === "Resource_ids" ? "styleCell":""}`}
              {...cell.getCellProps()}>
                {overrideCellsData.hasOwnProperty(
                  cell.column.Header
                ) ||
  ...
  const onPreviousPage = useCallback(
    (currentPage) => {
      if (isAPIPaginationEnabled) {
        ...
        apiPaginationConfig.onPreviousPage(currentPage - 1, pageSize, searchInput);
      }
      eventListeners.onPreviousPage(currentPage - 2);
      previousPage();
    },
    [previousPage, isAPIPaginationEnabled, apiPaginationConfig, eventListeners, pageSize, searchInput]
  );
  const onNextPage = useCallback(
    ...
    [nextPage, isAPIPaginationEnabled, apiPaginationConfig, eventListeners, pageSize, searchInput]
  );
  const onPageSize = useCallback(
    ...
    [setPageSize, isAPIPaginationEnabled, apiPaginationConfig, pageIndex, eventListeners, gotoPage, updatePageIndex, searchInput]
  );
  const onSearchChange = useCallback(
    ...
    [setGlobalFilter, eventListeners, setSearchInput, isAPIPaginationEnabled, apiPaginationConfig]
  );
  ...
  const pageRecordsUpperLimit = useMemo(
    () => {
      ...
      return Math.min(pageIndex * +pageSize + +pageSize, isAPIPaginationEnabled ? apiPaginationConfig.numberOfRows : rows.length)
    },
  ...
  const onTableTitle = useMemo(
    ...
    [showRowActions, selectedFlatRows, tableTitle, toggleTableShown, isTitleCollapsable, implementRowSelection, implementSelectRowActionsDropdown, selectRowActions]
  );
  ...
  return (
     <>
      {/* {(tableTitle ||
        ...
        <Row className={`filter-pagination-container widget-card-header ${uniqueMobileCss}`}>
          <div className="table-title">
            {tableTitle && isTitleCollapsable && (
              <Icon> {tableShown ? "arrow_drop_down" : "arrow_drop_up"}</Icon>
            )}
            <SwitchTransition mode={"out-in"}>
              <CSSTransition
                key={`${showRowActions}-css-transition`}
            ...
            isSearchEnabled && (<></>
              ...
              >
                <InputGroup
                  ...
                  className={`${tableLoader && "disabled"}`}
                >
                  <InputGroup.Prepend>
                    <InputGroup.Text
                      id="basic-addon1"
                  ...
                  <FormControl
                    placeholder={`Search ${
                      isAPIPaginationEnabled ? "(Press Enter to search)" : ""
                    }`}
                    aria-label="Search"
                    ...
                    onKeyDown={({ keyCode }) => {
                      if (
                        isAPIPaginationEnabled &&
                        apiPaginationConfig?.onSearch &&
                        keyCode === 13
                      ) {
                        apiPaginationConfig.onSearch(1, pageSize, searchInput);
                      }
                    }}
                  />
                </InputGroup>
          ...
          <Col
            md={3}
            className={`pagination-container ${tableLoader && "disabled"}`}
            // style={{
            ustifyContent: (settingOptions && !isPaginationEnabled) ? "flex-end" : ''}}
          >
          <div className={`pagination-container ${tableLoader && "disabled"}`}>
            {isPaginationEnabled && (
              <div className="pagination">
                <span
                  className={`pagination-btn ${!canPreviousPage && "disabled"}`}
                  onClick={() => onPreviousPage(pageIndex + 1)}
                >
                  <Icon>navigate_before</Icon>
                </span>
                <span
                  className="page-number font-16`"
                  key={`${pageSize}-${pageIndex}`}
                >
                  <strong>
                    {pageRecordsLowerLimit} - {pageRecordsUpperLimit}
                  </strong>{" "}
                  of{" "}
                  {isAPIPaginationEnabled
                    ? apiPaginationConfig?.numberOfRows
                    : rows.length}
                </span>
                <span
                  className={`pagination-btn ${!canNextPage && "disabled"}`}
            ...
            {OverrideSettingsComponent
              ? OverrideSettingsComponent()
              : (settingOptions || isPaginationEnabled || enableCsvExport) && (
                  <DropdownButton
                    id={"settings"}
                    variant={"outline-secondary"}
                    title={<Icon>settings</Icon>}
                  ...
                  >
                    ...
                    <Dropdown.Divider />
                    <Dropdown.Item
                      onSelect={() => setShowSelectColumnsModal(true)}
                    >
                      Select Columns
                    </Dropdown.Item>
                    {isPaginationEnabled && (
                      <>
                        <Dropdown.Divider />
                        <Dropdown.Item disabled> Number of Rows </Dropdown.Item>
                        {[10, 30, 50, 70, 100].map((item) => (
                          <Dropdown.Item
                            eventKey={item}
                            key={item}
                            onSelect={onPageSize}
                            active={Number(item) === Number(pageSize)}
                          >
                            {item}
                          </Dropdown.Item>
                        ))}
                        ...
                        >
                          ...
                          <div className={"header-filter-container"}>
                            <OverlayTrigger
                              ...
                              placement="bottom"
                              overlay={
                                <Tooltip id="tooltip-auto" style={!labelOnHover && {display:"none"}}>
                                  {column.label}
                                </Tooltip>
                              }>
...
TableWrapper.propTypes = {
  ...
  /*overrideCellsData: PropTypes.arrayOf(PropTypes.shape({
  headerAccessor: PropTypes.string,
  ...
  defaultSortedColumns: PropTypes.arrayOf(PropTypes.shape({
  ...
  defaultColumnFilters: PropTypes.arrayOf(PropTypes.shape({
  ...
  selectRowActions: PropTypes.arrayOf(PropTypes.shape({
